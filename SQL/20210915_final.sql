...
CREATE TABLE final.member (
   ...
   `memberBirth` TIMESTAMP NULL,
	...
	constraint member_memberEmail_uq UNIQUE KEY (memberEmail),
...
CREATE TABLE final.crew (
   ...
   `memberIdx` INTEGER NOT NULL,
    ...
    constraint fk_memberName_to_crewCapName foreign key (memberNickName) references final.member(memberNickName) ON DELETE CASCADE on update cascade
...
select count(*) as count, L.boardIdx
from final.like as L
group by boardIdx
;
...
select count(*), P.boardIdx, P.boardPhoto,P.boardDate, P.memberIdx
from  final.photoBoard as P left join final.like as L 
...
select  P.boardIdx, P.boardPhoto,P.boardDate, P.memberIdx, L.count
from final.photoBoard as P left outer join final.countLike as L 
on  L.boardIdx = P.boardIdx 
