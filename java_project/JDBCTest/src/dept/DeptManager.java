...
public class DeptManager {
	...
	void inputData (){
		...
		try {
			...
			Dept dept = new Dept(0, deptdata[0], deptdata[1]); //부서번호 임의로 0넣어도 상관없다. 

			int result = dao.insertDept(conn, dept);//몇개의 행이 입력되었는지 반환
	...
	//데이터 수정
	////사용자에게 Scanner 클래스로  데이터를 입력받아  -> dao edittDept() 메소드로 이용 
	void editData (){
		//Connection 객체 생성 -트랜젝션 처리
		Connection conn = null;
