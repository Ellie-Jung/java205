...
import ch04.CompanyContact;
import ch04.CustomerContact;
...
public class SmartPhone1 {
	...
	Contact1 [] con = new Contact1[10];

	//② 배열에 인스턴스를 저장하고, 수정하고, 삭제, 저장된 데이터의 리스트를 출력하는 메소드를 정의합니다
	void addInfo(Contact1 c) { //배열에 인스턴스 저장
		con[i++] = c;
	}
	void editInfo(){ //배열 수정
	...
	void deleteInfo() {//배열의 요소를 삭제
		...
		for(int i =0; i<con.length;i++) {
			if(name.equals(con[i].getName())){
				...
				con[con.length-1] = new CustomerContact();
			}
...
//			smartPhone.addInfo(c1);
			Contact1 c2 = new CompanyContact(name,phoneNum,email,address,birthday,group);
